...
function SidebarTabs({
  ...
  return (
    <div className='md:block hidden'>
      <div className="flex fixed flex-col     bg-yellow-50 z-40 w-0  md:min-w-[220px]">
        <div className="flex relative items-center border-b border-white justify-center h-12">
          <img
            onClick={() => navigate('/admin/dashboard')}
            src="https://colonyrestaurant.co.uk/wp-content/uploads/2020/04/colony_logo.png?v=25.4.3.0"
            alt="logo"
            className="h-8 cursor-pointer"
          />
          <div className="h-8 w-8     bg-yellow-50 flex items-center justify-center absolute -right-4 rotate-45">
            <MdMenu className="text-black -rotate-45 cursor-pointer" />
          </div>
        </div>
        <div className="overflow-y-auto h-[calc(100vh-3rem)]">
          <List className="pt-2 text-black">
              ...
              <div key={idx}>
                ...
                >
                  <div className="flex flex-row items-center gap-2">
                    ...
                    {item?.icons && <div>{idx === 0 ? <BiSolidDashboard /> : idx === 1 ? <FaUsers /> : <FaTicketAlt />} </div>}
                  ...
                  {item.children && (
                    openSections[item.title] ? (
                      ...
                      <HiMiniArrowTrendingDown className="text-black" />
                    )
                  )}
                </ListItemButton>
                      ...
                      {item.children.map((child, cIdx) => (
                        ...
                        >
                          <div className="relative">
                            <FaCircle
                              className={`text-[8px] ${child?.isActive ? 'text-black' : 'text-black'
                                }`}
                            />
                          ...
                          <div
                            ...
                            className={`text-[12px] ${child?.isActive ? "bg-white text-blue100" : "text-black"}  h-full font-semibold !flex w-full rounded-l-full pl-2 !items-center`}
                          >
                            {child.title}
                          </div>
                        </ListItemButton>
